...
watch(() => props.selectedMemberDetails, (newContent) => {
  if (newContent) {
    ...
    formData.name = newContent.name;
    formData.role = newContent.role;
...
const handleSave = () => {
  ...
  emit('submitMemberDetailsChanges', updatedContent);
};
const handleDelete = () => {
  emit('removeMemberDetails');
};
const closePopup = () => {
  emit('hideMemberDetailsPopup');
};
</script>
